...
public class Listnersclass implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		TakesScreenshot sh = (TakesScreenshot) com.Driverfactory.Browser.driver;
		File src = sh.getScreenshotAs(OutputType.FILE);
		File drs = new File(".//Screenshots//" + result.getName() + ".png");
		try {
			Files.copy(src, drs);
			Allure.addAttachment(result.getName(),
					new ByteArrayInputStream(((TakesScreenshot) Browser.driver).getScreenshotAs(OutputType.BYTES)));
		} catch (IOException e) {
